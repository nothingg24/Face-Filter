...
class SoftWingLoss(Metric):
    ...
    def __init__(self, omega1: float = 2.0, omega2: float = 20.0, epsilon: float = 0.5,  **kwargs: Any) -> None:
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        delta = (target - preds).abs()
        sum_abs_error = torch.where(delta < self.omega1, delta, self.omega2 * torch.log(1.0 + delta / self.epsilon) + self.B)
        error = sum_abs_error.sum(dim=[1, 2])
